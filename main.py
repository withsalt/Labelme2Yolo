...
def create_save_file(label_path):
    """
    按照训练时的图像和标注路径创建文件夹
    :param label_path:当前文件路径
    :return:
...
def push_into_file(file, images, labels, label_path, suffix='.jpg'):
    """
...
def json2txt(classes, txt_Name, label_path, suffix='.jpg'):
    """
    ...
    tempPath = os.path.join(label_path, 'tmp');
    if not os.path.exists(tempPath):
        os.makedirs(tempPath)

    list_file = open(f'{tempPath}/{txt_Name}.txt', 'w')
...
def ChangeToYolo5(label_path, suffix='.jpg', test_size=0.1, isUseTest=False):
    """
    生成最终标准格式的文件
    :param test_size: 分割测试集或验证集的比例
    :param label_path:当前文件路径
    ...
    classes = get_all_class(files,label_path)
    json2txt(classes,"all", label_path)
    ...
    push_into_file(train_files, train_image, train_label, label_path,suffix=suffix)
    push_into_file(val_files, val_image, val_label, label_path,suffix=suffix)
    if test_file is not None:
        push_into_file(test_file, test_image, test_label, label_path,suffix=suffix)
    print('create dataset done')


if __name__ == "__main__":
    ChangeToYolo5("D:\\openai\\dino\\train\\test\\images")